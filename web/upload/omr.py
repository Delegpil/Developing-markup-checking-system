...
def draw_point(point, img, radius=5, color=(255, 0, 0)):
...
def get_answers(source_file):
    """Run the full pipeline:
        ...
        - Load image zurgiig unshin
        - Convert to grayscale unshisan zurgaa har tsagaan zuragruu hurwuulj bgaa 
        - Filter out high frequencies with a Gaussian kernel-in arga ashiglan zurgiin ontslog tseguudiig tanih arga eneni zurgiig bursilgej zurgand analyz hiideg 
        - Apply threshold zurgand bosgo togtooj zurgand analiz hiij tsewerledeg
        - Find contours zuragnaas bulan haih
        - Find corners among all contours zuragnaas asuultiin dugui dursuudiig haih
        - Find 'outmost' points of all corners
        - Apply perpsective transform to get a bird's eye view
        - Scan each line for the marked answer mur tus bureerni hailt hiij duguilsan hesgiin utgiig haij olno
    """
    ...
    #cv2.imwrite('jinhene1.png', im_orig)
    #cv2.imwrite('Gaussian2.png', blurred)
    #cv2.imwrite('im3.png', im)
    #cv2.imwrite('perspective4.png', transf)
...
def main():
    ...
    parser.add_argument(
        "--input",
        help="Oruulah zurgiin neriig oruulna uu",
        required=True,
        type=str)

    parser.add_argument(
        "--output",
        help="Gargah zurgiin neriig oruulna uu",
        type=str)
    ...
    cv2.imwrite(args.input, rotated)
    cv2.waitKey(0)
    ...
    for i, answer in enumerate(answers):
      ...
      #  query = """INSERT INTO image(id,  asuult_id, asuult_hariu) VALUES (null, '%s', '%s')"""
       # query = "INSERT INTO image(id,  asuult_id, asuult_hariu) VALUES (null, '%s', '%s')" % \('Mohan', 20, 'M', 2000)
        #query =('insert into image(asuult_id, asuult_hariu) values ("%s", "%s")' % \(i+1, answer))
        querty1 = "select * from students WHERE image_path='%s'" %(args.input)
       # result = db.select(querty1)

        cursor = con.cursor()
        try:
            cursor.execute(querty1)
            result = cursor.fetchall()
            for row in result:
                cipher = row[1]
                hicheel_code = row[2]
                soril = row[5]
         #   print "fname=%s,lname=%s, %s" % \
          #   (cipher, hicheel_code, soril )
        except:
            print "Error: unable to fecth data"
        #print gname   
        #print result
        query = "insert into student_answer VALUES(null, '%s', '%s', '%s', '%s', '%s', '%s')" % \
 (i+1, answer, args.input, cipher, hicheel_code, soril)
        db.insert(query)
    if args.output:
